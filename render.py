...
from gaussian_renderer import render, render_from_load
import torchvision
...
def render_fn(views, gaussians, pipeline, background, use_amp):
    with torch.autocast(device_type='cuda', dtype=torch.float16, enabled=False):
        for view in views:
            render_from_load(view, gaussians, pipeline, background, "RGB")["render"]
...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background):
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        rendering = render_from_load(view, gaussians, pipeline, background, "RGB")["render"]
...
def render_sets(dataset: ModelParams, opt: OptimizationParams, pipeline: PipelineParams, qp:QuantizeParams, opt_lod, args,
    ...
    with torch.no_grad():
        ...
        gaussians = GaussianModel(dataset.sh_degree, qp, dataset, opt_lod)
...
if __name__ == "__main__":
    ...
    render_sets(lp_args, op, pp_args, qp_args, op_lod, args, args.skip_train, args.skip_test)
