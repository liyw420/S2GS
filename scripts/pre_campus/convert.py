...
parser.add_argument("--camera", default="OPENCV", type=str)                    # 对于非高质量相机，选PINHOLE的模式
...
# # 创建dense目录
dense_dir = args.source_path
# if not os.path.exists(dense_dir):
#     os.makedirs(dense_dir + "/sparse")
# files = os.listdir(args.source_path + "/sparse/0")
# # Copy each file from the source directory to the destination directory
# for file in files:
#     if file == '0':
#         continue
#     source_file = os.path.join(args.source_path, "sparse", "0", file)
#     destination_file = os.path.join(dense_dir, "sparse", file)
#     shutil.move(source_file, destination_file)
