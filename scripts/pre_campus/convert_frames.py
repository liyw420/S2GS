...
parser.add_argument("--camera", default="PINHOLE", type=str)            # 对于非高质量相机，选PINHOLE的模式
...
for id in range(2,args.last_frame_id+1):
    frame_id = f'{id:0>6}'
    ...
    if(args.resize):
        ...
        os.makedirs(os.path.join(args.source_path, inputDir[1:]) + "/images_4", exist_ok=True)
        ...
        for file in files:
            ...
            destination_file = os.path.join(args.source_path, inputDir[1:], "images_4", file)
            shutil.copy2(source_file, destination_file)
