...
import numpy as np
import re
from utils.graphics_utils import getWorld2View2, getProjectionMatrix, getProjectionMatrixOffCenter, getProjectionMatrixCenterShift
...
class Camera(nn.Module):
    def __init__(self, colmap_id, R, T, FoVx, FoVy, image, gt_alpha_mask,
                 ...
                 frame_idx = 0, znear=0.01, zfar=100.0, cx=0, cy=0, fl_x=0, fl_y=0
                 ):
...
class SequentialCamera(nn.Module):
    def __init__(self, colmap_id, R, T, FoVx, FoVy, uid, image_wh,
                 ...
                 znear=0.01, zfar=100.0, principle_point=None, cx=0, cy=0, fl_x=0, fl_y=0
                 ):
        ...
        self.world_view_transform = torch.tensor(getWorld2View2(R, T, trans, scale)).transpose(0, 1).cuda()
        if principle_point is None and cx<0:
            self.projection_matrix = getProjectionMatrix(znear=self.znear, zfar=self.zfar, fovX=self.FoVx, fovY=self.FoVy).transpose(0,1).cuda()
        elif cx > 0:
            self.projection_matrix = getProjectionMatrixCenterShift(self.znear, self.zfar, cx, cy, fl_x, fl_y, self.image_width, self.image_height).transpose(0,1).cuda()
        else:
