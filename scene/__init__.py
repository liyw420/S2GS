...
class Scene:
    ...
    def __init__(self, args : ModelParams, gaussians : GaussianModel, train_image_data=None, test_image_data=None,
        ...
        if os.path.exists(os.path.join(args.source_path, "sparse")):
            ...
            scene_info = sceneLoadTypeCallbacks["Technicolor"](args.source_path, args.test_indices, N_video_views)
        ...
        if self.loaded_iter:
            ...
            self.gaussians.create_from_pcd(scene_info.point_cloud, self.cameras_extent, self.train_cameras, resolution_scales)
            if verbose:
    ...
    def resetCameraCache(self):
        for camera in self.getTrainCameras():
            camera.colors_precomp = torch.zeros(self.gaussians._offset.shape[0],3).to(self.gaussians._features_dc)
