...
def get_depth_poses(pose_file):
    ...
    intrinsics = np.eye(3)
    intrinsics[0,2] = H/scale
    intrinsics[1,2] = W/scale
    intrinsics[0,0] = focal/scale
    intrinsics[1,1] = focal/scale

    # Convert to homogeneous coordinates
    poses = poses[:,:,:-1]
    poses = np.concatenate((poses,np.zeros((poses.shape[0],1,poses.shape[2]))),axis=1)
    poses[:,3,3] = 1.0
...
class Camera:
  """Camera class for handling 3D geometry, projection, and distortion correction."""
  ...
  def from_json(cls, path: PathType):
    ...
    # Handle legacy JSON format compatibility
  ...
  def pixel_to_local_rays(self, pixels: np.ndarray):
    """Convert pixel coordinates to normalized local ray directions."""
  ...
  def pixels_to_rays(self, pixels: np.ndarray) -> np.ndarray:
    """Convert pixel coordinates to world-space ray directions.
  ...
  def pixels_to_points(self, pixels: np.ndarray, depth: np.ndarray):
    """Convert pixel coordinates and depth to 3D world points."""
  ...
  def points_to_local_points(self, points: np.ndarray):
    """Transform world points to camera-local coordinate system."""
    translated_points = points - self.position
    local_points = (np.matmul(self.orientation, translated_points.T)).T
    return local_points

  def project(self, points: np.ndarray):
    """Project 3D world points to 2D pixel coordinates with distortion."""
  ...
  def get_pixel_centers(self):
    """Generate pixel center coordinates for the entire image."""
  ...
  def scale(self, scale: float):
    """Create a scaled version of the camera with adjusted intrinsics."""
    if scale <= 0:
